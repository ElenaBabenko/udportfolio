...
var pizzaElementGenerator = function(i) {
  ...
  // adding a class that controls pizza resizing on the parent element
  pizzaResize = document.querySelector("#randomPizzas");

  pizzaContainer.classList.add("randomPizzaContainer");
  // pizzaContainer.style.width = "33.33%";         // removing to add it thru stylesheet
  // pizzaContainer.style.height = "325px";         // removing to add it thru stylesheet
  pizzaContainer.id = "pizza" + i;                  // gives each pizza element a unique id
  pizzaImageContainer.classList.add("col-xs-5");    // changed column size for better visual layout
  ...
  pizzaDescriptionContainer.classList.add("col-xs-7"); // changed column size for better visual layout
...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    // creating a variable to cache the element
    var sizeLabel = document.getElementById("pizzaSize");
  ...
  // Setting style class on a parent element to resize randomPizzaContainers
  function changePizzaSizes(size) {
    var resizeDiv, widthClassName, arrayWidthClasses, i;

    // variable that stores the array of all class names for sizing pizzas
    arrayWidthClasses = ["small-size","medium-size","large-size"];
    //variable to cache location of the target element
    resizeDiv = document.querySelector("#randomPizzas");

    switch(size) {
      case "1":
        widthClassName = arrayWidthClasses[0]; // assigning a class name to a variable that will pass it on into the for loop below
        break;
      case "2":
        widthClassName = arrayWidthClasses[1];
        break;
      case "3":
        widthClassName = arrayWidthClasses[2];
        break;
      default:
        console.log("bug in sizeSwitcher");
    }

    // iterating thru all the class names in the array to see if it's already present in the element's class property
    // if it is, remove it before adding the new size (widthClassName) that is set by the slider
    for (i = 0; i < arrayWidthClasses.length; i++) {
      if (resizeDiv.classList.contains(arrayWidthClasses[i]) === true) {
        resizeDiv.classList.remove(arrayWidthClasses[i]);
        resizeDiv.classList.add(widthClassName);
      }
    }
  }
...
function updatePositions() {
  // caching all new variables needed in the function
  var scrollSpeed, constArray, numPizzas, phase, items, i;

  frame++;
  window.performance.mark("mark_start_frame");

  // Speed of the scroll on the screen
  scrollSpeed = (document.body.scrollTop / 1250);
  // caching the array of the pizza images that we will apply transform to
  items = document.getElementsByClassName("mover");
  // caching number of images to transform
  numPizzas = items.length;

  // iterating over the pizza images and appling transformation to each one
  for (i = 0; i < numPizzas; i++) {
    // phase is calculating distance values that cause pizzas to move sideways on scroll
    phase = Math.sin(scrollSpeed + (i % 5));
    // using transform style instead of left to decrease painting time
    // 100 seems to be a "magic number" that controls to what degree each pizza image moves to the left and right
...
document.addEventListener("DOMContentLoaded", function() {
  var colSpace, rowSpace, screenHeight, screenWidth, cols, rows, numMovingPizzas;

      // Using screen.availableHeight and width to make sure that there is enough pizzas for the largest window size possible
      screenHeight = screen.availHeight;
      screenWidth = screen.availWidth;

      // setting and calculating initial spacing between pizza images
      cols = 8;
      rowSpace = 256;
      colSpace = Math.round(screenWidth/cols); // space between columns depends on screen width
      rows = Math.round(screenHeight/rowSpace);   // number of rows depends on screen height


      //calculating the total number of pizzas on the screen
      numMovingPizzas = Math.ceil(cols * rows);
      console.log("rows: " + rows + ", cols: " + cols + ", colSpace: " + colSpace + ", rowSpace: " + rowSpace + ", numMovingPizzas: " + numMovingPizzas);

  for (var i = 0; i < numMovingPizzas; i++) {
    var elem = document.createElement("img");
    elem.className = "mover";
    elem.src = "images/pizza.png";
    // elem.style.height = "100px";     // removing to set it in the stylesheet since this is statis number
    // elem.style.width = "73.333px";   // removing to set it in the stylesheet since this is statis number

    // this seems to calculate the initial distance between images
    // 0.65 is a "magic number" that forces all image to display on the page, at least on the large screen
    elem.basicLeft = (i % cols) * (colSpace * 0.65);
    elem.style.top = (Math.floor(i / cols) * rowSpace) + "px"; // calculating the top property for each pizza based on row spacing
