...
function getNoun(y) {
  switch(y) {
    case "animals":
      ...
      "gorilla", "monkey", "seahorse", "wombat", "wolf", "giraffe", "badger", "lion", "bat", "mongoose", "swan", "firefly", "platypus"];
      return animals;
    case "profession":
...
var adjectives = ["whimsical", "noise", "apocalyptic", "insulting", "praise", "scientific"];  // types of adjectives for pizza titles
var nouns = ["animals", "fantasy", "gross", "horror", "places", "scifi"];                        // types of nouns for pizza titles
...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    var sizeLabel = document.querySelector("#pizzaSize").innerHTML;
  ...
  // Cameron"s solution from the class, I think I don"t need to improve on this
  function changePizzaSizes(size) {
    ...
    var pizzaElems = document.getElementsByClassName("pizza-image");

    for (var i = 0; i < pizzaElems.length; i++) {
      pizzaElems[i].style.transform = "scale(" + newWidth + ")";
    }
  }
...
document.addEventListener("DOMContentLoaded", function() {
  var vertSpace, horSpace, screenHeight, screenWidth, cols, rows, numMovingPizzas;

      /* Tried window.innerHeight and width but it doesn"t refresh number of pizzas on window resize.
         So using screen.availableHeight and width to make sure that there is enough pizzas
         for the largest window size possible */
      screenHeight = screen.availHeight;
      screenWidth = screen.availWidth;

      // setting initial spacing between pizza images
      rows = 5;
      horSpace = 300;
      vertSpace = Math.round(screenHeight/rows);
      cols = Math.round(screenWidth/horSpace);

      //number of pizzas on the screen
  ...
  for (var i = 0; i < numMovingPizzas; i++) {
    ...
    elem.style.top = (Math.floor(i / cols) * vertSpace) + "px";
